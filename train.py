...
from leaf.fusion import GroupFusion
import diffusers
...
from src.util.loss import cosine_loss, seg_aware_cosine_loss
from src.util.metric import SegmentationMetric, Visualization
from src.util.seeding import generate_seed_sequence
from src.util.seeding import seed_all   # 已经 import 了，直接用
import torch.backends.cudnn as cudnn
logger = get_logger(__name__, log_level="INFO")
import os
os.environ.setdefault("CUDA_DEVICE_ORDER", "PCI_BUS_ID")
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "3")  # 进程里只看到这张卡

import torch
if torch.cuda.is_available():
    torch.cuda.set_device(0)  # 注意：这里的0是“相对索引”，对应物理3号
    print("CUDA_VISIBLE_DEVICES =", os.environ.get("CUDA_VISIBLE_DEVICES"))
    print("torch sees", torch.cuda.device_count(), "GPU(s). current_device =", torch.cuda.current_device())
...
def load_dinov2(model_name: str, resolution: int = 256) -> nn.Module:
    ...
    # 本地 dinov2 仓库路径 —— 就是你截图里的这个目录
    # /home/24dxq/.cache/torch/hub/facebookresearch_dinov2_main
    local_repo = os.path.expanduser("~/.cache/torch/hub/facebookresearch_dinov2_main")

    # 关键：source="local" + 本地路径
...
def log_validation(
    ...
    pipeline = LeafPipeline(
        ...
        fusion=accelerator.unwrap_model(fusion),  # 新增
    )
...
def main():
    ...
    # ====== 新增：C 位置的分组融合模块 ======
    fusion = GroupFusion(in_channels=8, out_channels=8, groups=4)
    # ====== 新增：C 位置的边界增强注意力融合模块 ======
    #fusion = GroupFusion(in_channels=8, out_channels=8)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for i, model in enumerate(models):
                ...
                # ====== 新增：保存分组融合模块 ======
                elif isinstance(model, GroupFusion):
                    model.save_pretrained(os.path.join(output_dir, "fusion"))
    ...
    train_dataloader = DataLoader(
        ...
        worker_init_fn=lambda wid: seed_all(cfg.seed + wid),
    ...
    (
        ...
        unet_wrapper, latent_encoder, fusion, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(max_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet_wrapper):
                ...
                # ====== 使用分组融合替代简单 concat ======
                cat_latents = fusion(rgb_latent, noisy_latents)   # [B, 8, h, w]
                ...
                if cfg.use_alignment:
                    with torch.no_grad():
                        # 1) DINO 的图像输入
                        rgb_for_dino = F.interpolate(
                            rgb, 224 * (cfg.resolution // 256), mode="bicubic"
                        )
                        rgb_for_dino = imagenet_transforms(rgb_for_dino)
                        z: torch.Tensor = vision_encoder.forward_features(
                            rgb_for_dino.to(weight_dtype)
                        )["x_norm_patchtokens"]  # [B, 256, C]

                        # 2) 把 GT mask 对齐到 DINO 的 patch 网格
                        #   先把 3 通道 mask 压成单通道（取均值或取第 1 个通道都行）
                        mask_1ch = torch.mean(mask, dim=1, keepdim=True)  # [B, 1, 256, 256]
                        mask_for_dino = F.interpolate(
                            mask_1ch, size=rgb_for_dino.shape[-2:], mode="nearest"
                        )  # [B, 1, 224, 224]

                        B, _, H, W = mask_for_dino.shape
                        num_patches = 16  # 16x16=256
                        patch_h = H // num_patches  # 14
                        patch_w = W // num_patches  # 14

                        mask_patches = mask_for_dino.view(
                            B, 1, num_patches, patch_h, num_patches, patch_w
                        ).mean(dim=(3, 5))  # [B, 1, 16, 16]
                        mask_tokens = (mask_patches.view(B, -1) > 0.5).float()  # [B, 256]
                    #if global_step == 0 and accelerator.is_local_main_process:
                         #print("z:", z.shape, "z_tilde:", z_tilde.shape, "mask_tokens:", mask_tokens.shape)
                    # 3) 分割感知蒸馏：前景 token 权重大，背景权重小
                    distill_loss = seg_aware_cosine_loss(
                        z.float(),  # DINO token
                        z_tilde.float(),  # UNet token（来自 UNetModelWrapper）
                        mask_tokens,  # 哪些 token 属于病灶
                        fg_weight=2.0,
                        bg_weight=1.0,
                    )

                    # 这里 distill_loss 已经是一个标量，不需要再 .mean()
                    loss += cfg.lam * distill_loss
                        ...
                        if global_step % cfg.validation_steps == 0:
                            ...
                            if best_dice_score < metrics["dice"]:
                                ...
                                if accelerator.is_main_process:
                                    ...
                                    logger.info(f"mIoU at this step: {metrics['miou']:.4f}")
